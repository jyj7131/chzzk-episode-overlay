...
const HELP_TEXT: &str = "1. OBS에 텍스트(GDI+) 소스를 추가하고 이름을 정해두기 (예: 같이보기 화수)\r\n\
2. OBS → 도구 → WebSocket 서버 설정 → WebSocket 서버 사용 체크\r\n\
3. [서버 정보 표시] 버튼을 눌러 나오는 서버 IP / 포트 / 비밀번호를 입력\r\n   (원컴 환경에선 서버 IP 127.0.0.1 그대로 사용 가능)\r\n\
4. \"텍스트 소스 이름\" 칸에 1번에서 정한 이름을 똑같이 입력\r\n\
5. [연결하기] 버튼 클릭\r\n\r\n\
투컴에서 연결이 안 되면\r\n\
• 송출컴 Windows 방화벽에서 OBS 허용\r\n\
• OBS가 보여주는 IP는 추정값이라 틀릴 수 있음 → 송출컴에서 ipconfig로 IPv4 주소 확인";
const HELP_H: i32 = 214;
